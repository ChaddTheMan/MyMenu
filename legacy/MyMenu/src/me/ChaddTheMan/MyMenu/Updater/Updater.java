 ...
 */
package me.ChaddTheMan.MyMenu.Updater;
...
public class Updater {
    ...
    private void deleteOldFiles() {
        File[] list;
        File[] fileArray = list = this.listFilesOrError(this.updateFolder);
        int n = list.length;
        int n2 = 0;
        while (n2 < n) {
    ...
    private void moveNewZipFiles(String zipPath) {
        File[] list;
        File[] fileArray = list = this.listFilesOrError(new File(zipPath));
        int n = list.length;
        int n2 = 0;
        while (n2 < n) {
            ...
            if (dFile.isDirectory() && this.pluginExists(dFile.getName())) {
                ...
                File[] fileArray2 = dList;
                int n3 = dList.length;
                int n4 = 0;
                while (n4 < n3) {
                    File cFile = fileArray2[n4];
                    boolean found = false;
                    File[] fileArray3 = oList;
                    int n5 = oList.length;
                    int n6 = 0;
                    while (n6 < n5) {
    ...
    private boolean pluginExists(String name) {
        File[] plugins;
        File[] fileArray = plugins = this.listFilesOrError(new File("plugins"));
    ...
    private boolean versionCheck() {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(DELIMETER).length == 2) {
                String remoteVersion = title.split(DELIMETER)[1].split(" ")[0];
