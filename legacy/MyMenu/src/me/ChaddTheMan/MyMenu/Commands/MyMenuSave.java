...
public class MyMenuSave
implements CommandExecutor {
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (label.equalsIgnoreCase("mmsave")) {
            if (!(sender instanceof Player)) {
                sender.sendMessage(Errors.NOT_A_PLAYER);
                return true;
            }
            Player player = (Player)sender;
            if (!player.hasPermission("MyMenu.admin.save")) {
                player.sendMessage(Errors.NO_PERMISSION);
                return true;
            }
            MyMenu.updateToConfig();
            player.sendMessage(String.valueOf(Messages.PREFIX) + "Menus saved to config!");
        }
        return true;
    }
}
